...
__global__ void RandomInitialization(cudaTextureObjects *texture_objects, Camera *cameras, float4 *plane_hypotheses,  float4 *scaled_plane_hypotheses, float *costs,  float *pre_costs,  curandState *rand_states, unsigned int *selected_views, const PatchMatchParams params)
{
...
__device__ void CheckerboardPropagation(const cudaTextureObject_t *images, const cudaTextureObject_t *depths, const Camera *cameras, float4 *plane_hypotheses, float *costs, float *pre_costs, curandState *rand_states, unsigned int *selected_views, const int2 p, const PatchMatchParams params, const int iter)
{
    int width = cameras[0].width;
    int height = cameras[0].height;
    if (p.x >= width || p.y >= height) {
    ...
    int left_1 = center - 1;
    int left_2 = center - 3;
    int right_1 = center + 1;
    int right_2 = center + 3;
    int up_1 = center - width;
    int up_2 = center - 3 * width;
    int down_1 = center + width;
    int down_2 = center + 3 * width;

    int window_size = 45; // length in one side

    // Adaptive Checkerboard Sampling
    float cost_array[8][32] = {2.0f};
    // 0 -- up_1, 1 -- up_2, 2 -- down_1, 3 -- down_2, 4 -- left_1, 5 -- left_2, 6 -- right_1, 7 -- right_2
    bool flag[8] = {false};
    ...
    if (p.y > 0) {
        ...
        if (p.y > 2) {
            flag[1] = true;
            costSecondMin = costs[up_2];
            costSecondMinPoint = up_2;
        }
        else if (p.y > 1 && p.x > 0) {
            flag[1] = true;
            up_2 = center - 2 * width - 1;
            costSecondMin = costs[up_2];
            costSecondMinPoint = up_2;
        }
        else if (p.y > 1) {
        ...
        if (flag[1] != true) {
            ComputeMultiViewCostVector(images, cameras, p, plane_hypotheses[up_1], cost_array[0], params);
        }
        else {
            for (int i = 1; i <= (window_size - 1) / 2; ++i) {
                if (p.y > 2 * i) {
                    int pointTemp = center - (2*i + 1) * width;
                    if (costs[pointTemp] < costMin) {
                        costSecondMin = costMin;
                        costSecondMinPoint = costMinPoint;
                        costMin = costs[pointTemp];
                        costMinPoint = pointTemp;
                    }
                    else if (costs[pointTemp] < costSecondMinPoint) {
                        costSecondMin = costs[pointTemp];
                        costSecondMinPoint = pointTemp;
                    }
                }
            }
            for (int i = 2; i <= window_size; ++i) {
                for (int j = 1; j <= i / 2; ++j) {
                    if (p.y > i - 1 && p.x > i - 4 + 2*j) {
                        int pointTemp = center - i * width - i - 1 + 2*j;
                        if (costs[pointTemp] < costMin) {
                            costSecondMin = costMin;
                            costSecondMinPoint = costMinPoint;
                            costMin = costs[pointTemp];
            ...
            up_1 = costMinPoint;
            up_2 = costSecondMinPoint;
            ComputeMultiViewCostVector(images, cameras, p, plane_hypotheses[up_1], cost_array[0], params);
            ComputeMultiViewCostVector(images, cameras, p, plane_hypotheses[up_2], cost_array[1], params);
        }
    }
    ...
    if (p.y < height - 1) {
        flag[2] = true;
        costMin = costs[down_1];
        costMinPoint = down_1;
        num_valid_pixels++;
        if (p.y < height - 3) {
            flag[3] = true;
            costSecondMin = costs[down_2];
            costSecondMinPoint = down_2;
        }
        else if (p.y < height - 2 && p.x > 0) {
            flag[3] = true;
            down_2 = center + 2 * width - 1;
            costSecondMin = costs[down_2];
            costSecondMinPoint = down_2;
        }
        ...
        if (flag[1] != true) {
            ComputeMultiViewCostVector(images, cameras, p, plane_hypotheses[down_1], cost_array[2], params);
        }
        else {
            for (int i = 1; i <= (window_size - 1) / 2; ++i) {
                if (p.y < height - 2 * i - 1) {
            ...
            for (int i = 2; i <= window_size; ++i) {
                for (int j = 1; j <= i / 2; ++j) {
                    ...
                    if (p.y < height - i && p.x < width - i - 2*j +3) {
                        int pointTemp = center + i * width + i + 1 - 2*j;
                        if (costs[pointTemp] < costMin) {
                            costSecondMin = costMin;
                            costSecondMinPoint = costMinPoint;
            ...
            down_1 = costMinPoint;
            down_2 = costSecondMinPoint;
            ComputeMultiViewCostVector(images, cameras, p, plane_hypotheses[down_1], cost_array[2], params);
            ComputeMultiViewCostVector(images, cameras, p, plane_hypotheses[down_2], cost_array[3], params);
        }
    }
    ...
    if (p.x > 0) {
        flag[4] = true;
        costMin = costs[left_1];
        costMinPoint = left_1;
        num_valid_pixels++;
        if (p.x > 2) {
            flag[5] = true;
            costSecondMin = costs[left_2];
            costSecondMinPoint = left_2;
        }
        else if (p.x > 1 && p.y > 0) {
            flag[5] = true;
            left_2 = center - width - 2;
            costSecondMin = costs[left_2];
            costSecondMinPoint = left_2;
        }
        else if (p.x > 1) {
        ...
        if (flag[5] != true) {
            ComputeMultiViewCostVector(images, cameras, p, plane_hypotheses[left_1], cost_array[4], params);
        }
        else {
            for (int i = 1; i <= (window_size - 1) / 2; ++i) {
                if (p.x > 2 * i) {
                    int pointTemp = center - (2*i + 1);
                    if (costs[pointTemp] < costMin) {
                        costSecondMin = costMin;
                        costSecondMinPoint = costMinPoint;
                        costMin = costs[pointTemp];
            ...
            for (int i = 2; i <= window_size; ++i) {
                for (int j = 1; j <= i / 2; ++j) {
                    if (p.x > i - 1 && p.y > i - 4 + 2*j) {
                        int pointTemp = center - i + (2*j - i - 1) * width;
                        if (costs[pointTemp] < costMin) {
                            costSecondMin = costMin;
                            costSecondMinPoint = costMinPoint;
                            costMin = costs[pointTemp];
                    ...
                    if (p.x > i - 1 && p.y < height - i - 2*j +3) {
                        int pointTemp = center - i + (i + 1 - 2*j) * width;
                        if (costs[pointTemp] < costMin) {
                            costSecondMin = costMin;
                            costSecondMinPoint = costMinPoint;
                            costMin = costs[pointTemp];
            ...
            left_1 = costMinPoint;
            left_2 = costSecondMinPoint;
            ComputeMultiViewCostVector(images, cameras, p, plane_hypotheses[left_1], cost_array[4], params);
            ComputeMultiViewCostVector(images, cameras, p, plane_hypotheses[left_2], cost_array[5], params);
        }
    }
    ...
    if (p.x < width - 1) {
        flag[6] = true;
        costMin = costs[right_1];
        costMinPoint = right_1;
        num_valid_pixels++;
        if (p.x < width - 3) {
            flag[7] = true;
            costSecondMin = costs[right_2];
            costSecondMinPoint = right_2;
        }
        else if (p.x < width - 2 && p.y > 0) {
            flag[7] = true;
            right_2 = center - width + 2;
            costSecondMin = costs[right_2];
            costSecondMinPoint = right_2;
        }
        else if (p.x < width - 2) {
        ...
        if (flag[7] != true) {
            ComputeMultiViewCostVector(images, cameras, p, plane_hypotheses[right_1], cost_array[6], params);
        }
        else {
            for (int i = 1; i <= (window_size - 1) / 2; ++i) {
                if (p.x < width - 2 * i - 1) {
            ...
            for (int i = 2; i <= window_size; ++i) {
                for (int j = 1; j <= i / 2; ++j) {
                    ...
                    if (p.x < width - i && p.y < height - i - 2*j +3) {
                        int pointTemp = center + i + (i + 1 - 2*j) * width;
                        if (costs[pointTemp] < costMin) {
                            costSecondMin = costMin;
                            costSecondMinPoint = costMinPoint;
            ...
            right_1 = costMinPoint;
            right_2 = costSecondMinPoint;
            ComputeMultiViewCostVector(images, cameras, p, plane_hypotheses[right_1], cost_array[6], params);
            ComputeMultiViewCostVector(images, cameras, p, plane_hypotheses[right_2], cost_array[7], params);
        }
    }
    const int positions[8] = {up_1, up_2, down_1, down_2, left_1, left_2, right_1, right_2};

    bool neighbor_flag[4] = {false};

    if (p.y > 0) {
        neighbor_flag[0] = true;
    }
    if (p.y < height - 1) {
        neighbor_flag[1] = true;
    }
    if (p.x > 0) {
        neighbor_flag[2] = true;
    }
    if (p.x < width - 1) {
        neighbor_flag[3] = true;
    }
    ...
    float bad_cost_threshold = 1.2 * expf((iter) * (iter) / (-120.0f));
    for (int i = 0; i < params.num_images - 1; i++) {
        float count = 0;
        int count_false = 0;
        float tmpw = 0;
        for (int j = 0; j < 8; j++) {
            ...
            if (cost_array[j][i] > bad_cost_threshold) {
                count_false++;
            }
        }
    ...
    PlaneHypothesisRefinement(images, depths, cameras, &plane_hypotheses_now, &depth_now, &cost_now, &rand_states[center], view_weights, weight_norm, p, params);
...
__global__ void BlackPixelUpdate(cudaTextureObjects *texture_objects, cudaTextureObjects *texture_depths, Camera *cameras, float4 *plane_hypotheses, float *costs,  float *pre_costs,  curandState *rand_states, unsigned int *selected_views, const PatchMatchParams params, const int iter)
{
    ...
    CheckerboardPropagation(texture_objects[0].images, texture_depths[0].images, cameras, plane_hypotheses, costs, pre_costs,  rand_states, selected_views, p, params, iter);
}
